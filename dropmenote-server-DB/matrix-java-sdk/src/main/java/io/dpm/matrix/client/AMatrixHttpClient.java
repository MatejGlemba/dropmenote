...
import io.dpm.matrix.client.api.AutoDiscoverySettings;
import io.dpm.matrix.client.api.MatrixClientRaw;
import io.dpm.matrix.MatrixErrorInfo;
import io.dpm.matrix.api.MatrixID;
import io.dpm.matrix.hs.api.MatrixHomeserver;
...
public abstract class AMatrixHttpClient implements MatrixClientRaw {
    ...
    public AMatrixHttpClient(MatrixClientContext context, OkHttpClient.Builder client) {
    ...
    public AMatrixHttpClient(MatrixClientContext context, OkHttpClient client) {
        this.context = context;
        this.client = client;
    }

    @Override
    public Optional<AutoDiscoverySettings> discoverSettings() {
    ...
    @Override
    public Optional<MatrixID> getUser() {
        return context.getUser();
    }

    public Request.Builder addAuthHeader(Request.Builder builder, String token) {
        builder.addHeader("Authorization", "Bearer " + token);
        return builder;
    }

    public Request.Builder addAuthHeader(Request.Builder builder) {
        return addAuthHeader(builder, getAccessTokenOrThrow());
    }

    public String executeAuthenticated(Request.Builder builder, String token) {
        return execute(addAuthHeader(builder, token));
    }

    public String executeAuthenticated(Request.Builder builder) {
        return execute(addAuthHeader(builder));
    }

    public String executeAuthenticated(MatrixHttpRequest matrixRequest) {
        addAuthHeader(matrixRequest.getHttpRequest());
        return execute(matrixRequest);
    }

    public String execute(Request.Builder builder) {
        return execute(new MatrixHttpRequest(builder));
    }

    public String execute(MatrixHttpRequest matrixRequest) {
    ...
    public MatrixHttpContentResult executeContentRequest(MatrixHttpRequest matrixRequest) {
        log(matrixRequest.getHttpRequest());
        try (Response response = client.newCall(matrixRequest.getHttpRequest().build()).execute()) {
            int responseStatus = response.code();
     ...
     */
    public String handleError(MatrixHttpRequest matrixRequest, int responseStatus, MatrixErrorInfo info) {
    ...
    public Optional<String> extractAsStringFromBody(String body, String jsonObjectName) {
    ...
    public HttpUrl.Builder getIsBaseUrl() {
        return HttpUrl.get(context.getIsBaseUrl()).newBuilder();
    }

    public HttpUrl.Builder getPathBuilder(HttpUrl.Builder base, String... segments) {
    ...
    public HttpUrl.Builder getPathBuilder(String... segments) {
        return getPathBuilder(getHsBaseUrl(), segments);
    }

    public HttpUrl.Builder getIdentityPathBuilder(String... segments) {
        return getPathBuilder(getIsBaseUrl(), segments);
    }

    public URL getPath(String... segments) {
        return getPathBuilder(segments).build().url();
    }

    public URL getIdentityPath(String... segments) {
        return getIdentityPathBuilder(segments).build().url();
    }

    public HttpUrl.Builder getClientPathBuilder(String... segments) {
        String[] base = { "client", "r0" };
        segments = ArrayUtils.addAll(base, segments);
        return getPathBuilder(segments);
    }

    public HttpUrl.Builder getMediaPathBuilder(String... segments) {
        String[] base = { "media", "r0" };
        segments = ArrayUtils.addAll(base, segments);
        return getPathBuilder(segments);
    }

    public URL getClientPath(String... segments) {
        return getClientPathBuilder(segments).build().url();
    }

    public URL getMediaPath(String... segments) {
        return getMediaPathBuilder(segments).build().url();
    }

    public RequestBody getJsonBody(Object o) {
    ...
    public Request.Builder getRequest(URL url) {
